...
static void processOneBlockPcbc(t_pcbcGenCtx *ctx, uint8_t *block)
{
	uint8_t tmp[64];
	...
	else
	{
		...
		ft_memcpy(ctx->prevPlain, block, ctx->blockSize);
	}
}
				   ...
				   size_t			*outLen)
{
	...
	*outLen = 0;
	...
	if (ctx->bufferLen > 0)
	{
		...
		processOneBlockPcbc(ctx, ctx->buffer);
